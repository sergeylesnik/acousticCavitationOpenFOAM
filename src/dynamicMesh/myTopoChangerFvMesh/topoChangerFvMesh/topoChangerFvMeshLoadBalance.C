...
bool Foam::topoChangerFvMesh::loadBalance(const dictionary& decompDict)
{
    ...
    HashTable<const volVector2Field*> volVector2Fields =
        thisDb().lookupClass<volVector2Field>();
    ...
    List<PtrList<volVector2Field> > receivedVolVector2Fields
    (
        volVector2Fields.size()
    );
    ...
    forAll (receivedVolVector2Fields, fieldI)
    {
        receivedVolVector2Fields[fieldI].setSize(Pstream::nProcs());
    }
    ...
    for (label procI = 0; procI < meshDecomp.nProcs(); procI++)
    {
        ...
        if (migratedCells[Pstream::myProcNo()][procI] > 0)
        {
            ...
            if (procI != Pstream::myProcNo())
            {
                ...
                sendFields(volVector2Fields, fieldDecomposer, toProc);
